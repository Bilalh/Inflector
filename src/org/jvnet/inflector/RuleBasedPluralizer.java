...
/**
 * <p>
 * An implementation of {@link Pluralizer} that is implemented using an ordered list of {@link Rule}s. It is possible to specify a fallback
 * {@link Pluralizer} that is invoked if none of the rules match. This makes it easy to override some rules of another {@link Pluralizer}.
 * </p>
 * <p>
 * This class also preserves leading and trailing whitespace, so individual rules don't need to explicitly handle it. Case is also
 * preserved; that is, the output of all uppercase input is automatically uppercased, and the output of titlecase input is automatically
 * titlecased. This means rules can act in a case-insensitive manner.
 * </p>
 * <p>
...
public class RuleBasedPluralizer implements Pluralizer {
	...
	/**
	 * <p>
	 * Constructs a pluralizer that uses a list of rules then an identity {@link Pluralizer} if none of the rules match. This is useful to
	 * build your own {@link Pluralizer} from scratch.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Constructs a pluralizer that uses first a list of rules then a fallback {@link Pluralizer}. This is useful to override the behaviour
	 * of an existing {@link Pluralizer}.
	 * </p>
	 * 
	 * @param rules the rules to apply in order
	 * @param locale the locale specifying the language of the pluralizer
	...
	/**
	 * <p>
	 * Converts a noun or pronoun to its plural form for the given number of instances. If <code>number</code> is 1, <code>word</code> is
	 * returned unchanged.
	 * </p>
	 * <p>
	 * The return value is not defined if this method is passed a plural form.
...
//			System.out.println("Match "+ word);	
			...
			if (plural == null) {
//				System.out.println("Fallback"+ word);	
	...
	/**
	 * <p>
	 * Goes through the rules in turn until a match is found at which point the rule is applied and the result returned. If no rule matches,
...
//				System.out.println("using rule" + rule);
	...
	/**
	 * <p>
	 * Apply processing to <code>pluralizedWord</code>. This implementation ensures the case of the plural is consistent with the case of
	 * the input word.
	 * </p>
	 * <p>
	 * If <code>trimmedWord</code> is all uppercase, then <code>pluralizedWord</code> is uppercased. If <code>trimmedWord</code> is
